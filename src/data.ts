...
import type { Category, Question, RichTextSegment } from "./types";

type ImportedQuestion = Question & {
  imageKey?: string | null;
  details?: Question["details"] & {
    stemRich?: Question["stemRich"];
    annotatedStemRich?: NonNullable<Question["details"]>["annotatedStem"];
...
export const questions = (importedQuestions as unknown as ImportedQuestion[]).map((question) => {
  ...
  return {
    ...question,
    options,
    stemRich: question.stemRich ?? details?.stemRich,
    details: details ? {
      ...details,
      annotatedStem: details.annotatedStem ?? details.annotatedStemRich,
      practical: details.practical ?? details.practicalAnalysis,
    } : undefined,
    imageUrl: question.imageUrl ?? (question.imageKey ? `/api/media/${question.imageKey}` : null),
  };
});
...
const typeCountsFor = (categoryId: number) => Object.entries(questions.filter((question) => question.categoryId === categoryId).reduce<Record<string, { types: string[]; count: number }>>((counts, question) => {
  const label = question.type || "未分类";
  counts[label] ||= { types: [], count: 0 };
  counts[label].types.push(question.type);
  counts[label].count += 1;
  return counts;
}, {})).map(([label, value]) => ({ type: [...new Set(value.types)].join(","), label, count: value.count }));
const quantityTypeCounts = typeCountsFor(4);
const dataTypeCounts = typeCountsFor(5);
...
export const categories: Category[] = [
  ...
  { id: 4, slug: "math", name: "数量关系", shortName: "数量", description: "数学运算与数字推理", color: "#ef5da8", softColor: "#fdebf4", questionCount: 600, typeCounts: quantityTypeCounts },
  { id: 5, slug: "data", name: "资料分析", shortName: "资料", description: "基期、增长量、增长率、比重与综合分析", color: "#3b82f6", softColor: "#eaf2ff", questionCount: 600, typeCounts: dataTypeCounts }
];
