...
def answer_candidates(paragraphs: list[Paragraph], label: str, kind: str) -> list[int]:
    ...
    if kind == "quantity":
        ...
        for index in candidates:
            normalized = re.sub(r"公考最新资料、更新进度微信SKA674", "", paragraphs[index].text)
            normalized = re.sub(r"\s+", "", normalized)
            if deduped:
                previous = deduped[-1]
                previous_normalized = re.sub(r"公考最新资料、更新进度微信SKA674", "", paragraphs[previous].text)
                previous_normalized = re.sub(r"\s+", "", previous_normalized)
                if normalized == previous_normalized:
                    if "公考最新资料、更新进度微信SKA674" in paragraphs[previous].text and "公考最新资料、更新进度微信SKA674" not in paragraphs[index].text:
                        deduped[-1] = index
                    continue
...
def question_start(paragraphs: list[Paragraph], before: int, answer_at: int, expected: int) -> int:
    ...
    if not matches:
        any_number = re.compile(r"(?<!\d)\d{1,3}\s*[.]\s*")
        matches = [i for i in range(before, answer_at + 1) if any_number.search(paragraphs[i].text)]
    if not matches:
    ...
    if not matches:
        ...
        matches = [i for i in range(before, answer_at + 1) if any_number.search(paragraphs[i].text)]
    if not matches:
        raise ValueError(f"question anchor missing before answer {answer_at}")
    return matches[-1]
...
def extract_group_material(paragraphs: list[Paragraph], question_start_at: int, lower: int) -> str:
    """Return the source passage printed before the first question in a data set.

    The analysis workbook repeats each five-question set as: set heading, source
    passage, then questions. Notes from the preceding set are separated by that
    heading, so use the last heading as a stable boundary and keep only prose.
    """
    heading_re = re.compile(r"(?:练习题|S四IHA|SIHAI|资料分析)\s*\d*", re.I)
    headings = [i for i in range(lower, question_start_at) if any(token in paragraphs[i].text for token in ("\u7ec3\u4e60\u9898", "\u8d44\u6599\u5206\u6790", "SIHAI", "S\u56dbIHA"))]
    start = headings[-1] + 1 if headings else lower
    answer_pages = [i for i in range(start, question_start_at) if "全篇答案" in paragraphs[i].text]
    end = answer_pages[-1] if answer_pages else question_start_at
    ignored = re.compile(r"^(?:【全篇答案】|【参考答案|【题型分类|【实战解析|花生批注|资料分析\s*6?\s*0?\s*0?)")
    parts: list[str] = []
    for paragraph in paragraphs[start:end]:
        value = compact(paragraph.text)
        if not value or ignored.search(value):
            continue
        if NOTE in value or re.match(r"^\s*\d{1,3}\s*[.]", value):
            continue
        parts.append(value)
...
def import_doc(path: Path, kind: str, category_id: int, id_base: int) -> tuple[list[dict], dict]:
    ...
    if len(answers) < (598 if kind == "data" else 599):
    ...
    for index, answer_at in enumerate(answers):
        ...
        # The next answer anchor is a safer boundary than the printed number:
        # several source sets restart numbering and a few pages omit a number.
        next_start = next_answer
    ...
    group_size = 10 if kind == "quantity" else 20
