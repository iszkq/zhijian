...
def merge_analysis(path: Path, bases: list[dict[str, object]]) -> None:
    ...
    for index, (base, ordered) in enumerate(zip(bases, primary)):
        ...
        # Some sequence-ordering options are stored only in the annotated rich
        # text layer of the analysis workbook. Recover their visible text before
        # writing the canonical options_json used by the practice UI.
        if any(not value.strip() for value in option_values.values()):
            for rich_source in [base.get("annotatedOptionRich"), best_rich.get("annotatedOptionRich"), ordered.get("annotatedOptionRich")]:
                if not isinstance(rich_source, dict):
                    continue
                for label, rich in rich_source.items():
                    if label in {"A", "B", "C", "D"} and isinstance(rich, list):
